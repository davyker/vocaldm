...
# Removed disabled_train function to allow proper train mode switching
...
class LatentDiffusion(DDPM):
    ...
    def instantiate_first_stage(self, config):
        ...
        # Removed disabled_train assignment to allow proper training
    ...
    def get_learned_conditioning(self, c):
        if self.cond_stage_model.embed_mode == "bypass":
            # When in bypass mode, we skip the conditioning stage model
            # since the embeddings are already adapted to the right format
            # The embeddings should already be normalized and shaped correctly
            if isinstance(c, torch.Tensor):
                # Handle direct tensor embeddings
                return c
            elif isinstance(c, list) and isinstance(c[0], torch.Tensor):
                # Handle list of tensor embeddings
                return c[0]
    ...
    def get_input(
        ...
        if return_first_stage_encode:
            ...
            # Removed .detach() to maintain gradient flow
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ["caption", "coordinates_bbox"]:
                    ...
                    xc = batch
                elif cond_key == "qvim_embedding":
                    # Handle QVIM embeddings from the batch
                    # The QVIM embeddings are passed as the 7th element in the batch tuple
                    if len(batch) >= 7:
                        xc = batch[6]
                        if type(xc) == torch.Tensor:
                            xc = xc.to(self.device)
                    else:
                        raise ValueError("Batch does not contain QVIM embeddings at index 6")
                else:
                    # [bs, 1, 527]
